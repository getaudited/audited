...
var PersonWhere = struct {
	...
	ID:        whereHelperint{field: "\"people\".\"id\""},
	FullName:  whereHelperstring{field: "\"people\".\"full_name\""},
	CreatedAt: whereHelpertime_Time{field: "\"people\".\"created_at\""},
}
...
var (
	personAllColumns            = []string{"id", "full_name", "created_at"}
	personColumnsWithoutDefault = []string{"full_name"}
	personColumnsWithDefault    = []string{"id", "created_at"}
...
func People(mods ...qm.QueryMod) personQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"people\".*"})
	}
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(personPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert people, could not build conflict column list")
			}

			conflict = make([]string, len(personPrimaryKeyColumns))
			copy(conflict, personPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"people\"", updateOnConflict, ret, update, conflict, insert, opts...)
